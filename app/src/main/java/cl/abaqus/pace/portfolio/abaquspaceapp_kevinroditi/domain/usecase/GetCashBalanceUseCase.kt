...
class GetCashBalanceUseCase @Inject constructor(
    private val repository: WealthRepository
) {
    operator fun invoke(): Flow<Resource<BigDecimal>> = repository.getPortfolio().map { resource ->
        when (resource) {
            is Resource.Success -> Resource.Success(resource.data!!.cashBalance)
            is Resource.Error -> Resource.Error(resource.message!!, resource.data?.cashBalance)
            is Resource.Loading -> Resource.Loading(resource.data?.cashBalance)
        }
    }
}
